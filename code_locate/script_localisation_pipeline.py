...
sum_size = 15
nb_StdVar_threshold = 10
type_Threshold = type_threshold.THRESHOLD
n_connectivity = 26
...
d_holo_filtered = cp.zeros(shape = (cam_nb_pix_Y, cam_nb_pix_X), dtype = cp.float32)
d_fft_holo = cp.zeros(shape = (cam_nb_pix_Y, cam_nb_pix_X), dtype = cp.complex64)
d_fft_holo_filtered = cp.zeros(shape = (cam_nb_pix_Y, cam_nb_pix_X), dtype = cp.complex64)
...
for image in os.listdir(path):
    if (image.split('.')[-1].lower() == type_image.lower()):
        ...
        i_image += 1
        h_holo = read_image(os.path.join(path,image), cam_nb_pix_X, cam_nb_pix_Y)
        if display_images:
            display(h_holo)
        ...
        d_labels, number_of_labels = CCL3D(d_bin_volume_focus, d_volume_module, type_Threshold, threshold, n_connectivity)
        t4 = time.perf_counter()
        # print('number of objects located: ', number_of_labels)

        if display_images:
            display(projection_bool(d_bin_volume_focus, axis=0))
            display(projection_bool(d_bin_volume_focus, axis=1))
            display(projection_bool(d_bin_volume_focus, axis=2))
